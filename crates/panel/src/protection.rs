use crate::model::PanelProtectionSnapshot;
...
) -> Result<PanelProtectionSnapshot> {
    let root = project_root
        .as_ref()
        .canonicalize()
        .with_context(|| format!("canonicalize {}", project_root.as_ref().display()))?;
    let panel_output_root = root.join(".agentflow/panel");
    let git_dir = resolve_git_dir(&root);

    let Some(git_dir) = git_dir else {
        return Ok(PanelProtectionSnapshot {
            version: "panel-protection.v1".to_string(),
            project_root: root.display().to_string(),
            status: "ready".to_string(),
            panel_output_root: panel_output_root.display().to_string(),
            git_exclude_path: None,
            protected_by_info_exclude: true,
            writes_only_panel_output: true,
...
mod tests {
    ...
    fn protection_warns_when_git_exclude_missing_agentflow() {
        ...
        let snapshot = check_panel_git_protection(dir.path()).unwrap();
