...
rg -n "Project / Issue Status Model v0|ProjectStatus|IssueStatus|MilestoneDerivedProgress|canonicalStatus|legacy status|Product Feature Project 默认 status = active|Product Feature Issue 默认 status = todo|agentflow plan 新建 Issue 默认 status = todo|Milestone 不作为产品状态机|derived progress" docs/specs/project-issue-status-model-v0.md README.md ROADMAP.md docs/specs/mvp-spec.md docs/specs/goal-criteria-driven-mvp.md docs/validation/latest-verification-summary.md verification.md crates/core/src/lib.rs crates/cli/src/main.rs apps/desktop/src/App.tsx apps/desktop/src/types.ts >/dev/null
rg -n "Team / Project / Milestone / Issue Writers v0|TeamDraft|ProjectDraft|MilestoneDraft|IssueDraft|CreationPreview|CreationWriteSummary|agentflow team create|agentflow project create|agentflow milestone create|agentflow issue create|preview-default|explicit-write-flag|Project default status = draft|Issue default status = todo|Milestone status = 不写入|team_project_milestone_issue_writers" docs/specs/team-project-milestone-issue-writers-v0.md README.md ROADMAP.md docs/specs/mvp-spec.md docs/specs/goal-criteria-driven-mvp.md docs/validation/latest-verification-summary.md verification.md crates/core/src/lib.rs crates/cli/src/main.rs >/dev/null
