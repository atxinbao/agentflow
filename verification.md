...
## 2026-06-03 Figma Prompt Reader Polish

执行者：Codex

目标：

- 基于用户粘贴的“机构级量化交易系统”Figma AI 提示词，先形成需求文档。
- 修复 Project File Reader 对 `.txt` / 提示词类文本的展示分类。
- 让 Browser Preview mock 文件树能用于验证该类长文本需求 / 提示词文件。

结果：

- 新增 `docs/requirements/009-figma-prompt-reader-polish.md`。
- 更新 `docs/requirements/README.md` 和 `docs/requirements/next-requirements.md`。
- Project File Reader 现在会将文件名包含 `prompt` / `提示词` 的文件标为 `提示词文档`。
- Project File Reader 现在会将 `.txt` / `text` / `rst` / `adoc` 或 language=`text` 的普通文本标为 `文本`，不再误标为 `代码`。
- Browser Preview mock 文件树新增：
...
- Browser Preview 核对：pass。
  - 临时打开 `http://127.0.0.1:1421/`。
  - 文件列表可见 `institutional-quant-figma-prompt.txt`。
  - 打开后可见机构级量化交易系统提示词文本。
  - `docs/requirements` 下可见并可打开 `009-figma-prompt-reader-polish.md`。
