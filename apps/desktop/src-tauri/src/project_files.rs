...
pub(crate) fn load_project_files_snapshot(
    ...
    let entries = read_project_file_entries(&root, &root)?;
    let selected_path = preferred_project_file_selection(&entries);

    Ok(ProjectFilesSnapshot {
        version: "project-files.v0".to_string(),
...
fn read_project_file_entries(
    ...
    for entry_result in read_dir {
        ...
        entries.push(project_file_entry_from_path(root, &path, &metadata)?);
    }
...
fn project_file_entry_from_path(
    ...
    let children = if metadata.is_dir() {
        read_project_file_children(root, path)?
    } else {
        Vec::new()
    };
...
fn read_project_file_children(
    ...
    for entry_result in read_dir.take(DIRECTORY_CHILD_LIMIT) {
        let entry = entry_result.map_err(|error| error.to_string())?;
        let path = entry.path();
        let metadata = entry
            .metadata()
            .map_err(|error| format!("metadata {}: {error}", path.display()))?;
        children.push(ProjectFileChild {
            name: path
                .file_name()
                .and_then(|name| name.to_str())
                .unwrap_or("")
...
fn read_project_file_content(root: &Path, path: &Path) -> Result<ProjectFileContent, String> {
    ...
    if metadata.is_dir() {
        return Ok(ProjectFileContent {
            ...
            data_url: None,
            truncated: false,
            directory_children: read_project_file_children(root, path)?,
...
fn file_mime_type(name: &str, extension: Option<&str>) -> Option<String> {
    ...
    let mime = match extension.unwrap_or("").to_lowercase().as_str() {
        ...
        "rs" | "ts" | "tsx" | "js" | "jsx" | "css" | "html" | "sh" | "bash" | "zsh" | "txt" => {
            "text/plain"
        }
        "csv" => "text/csv",
        "pdf" => "application/pdf",
