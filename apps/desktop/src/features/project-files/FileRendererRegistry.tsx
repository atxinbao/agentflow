...
export function ProjectFileBodyRenderer({ content, entry }: { content: ProjectFileContent; entry: ProjectFileEntry | null }) {
  ...
  switch (renderer) {
    ...
    case "large-text":
      return <LargeTextReader content={textContent} reason={content.unsupportedReason} />;
    case "table":
...
function LargeTextReader({ content, reason }: { content: string; reason?: string | null }) {
  const viewportRef = useRef<HTMLDivElement | null>(null);
  const [scrollTop, setScrollTop] = useState(0);
  const lines = useMemo(() => (content || "暂无文本预览。").split("\n"), [content]);
  ...
  return (
    <div className="project-large-text-reader">
      ...
      <p className="project-large-text-status">
        大文本只读窗口：当前渲染第 {startLine + 1}-{Math.max(startLine + visibleLines.length, 1)} 行，共 {lines.length} 行，避免一次性把大文件塞进 DOM。
      </p>
      <div
        ref={viewportRef}
