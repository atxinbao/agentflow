...
function projection(status: WorkflowStatus): TaskProjection | null {
  ...
  return {
    ...
    delivery: {
      status: status === "done" ? "published" : "ready",
    ...
    runtime: {
      runId: `run-${status}`,
      runStatus: status === "in_progress" ? "in_progress" : "completed",
      branchName: `agentflow/workflow/${status}`,
      checkpointCount: status === "in_progress" ? 2 : 3,
      latestCheckpointId: "checkpoint-003",
