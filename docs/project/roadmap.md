...
| `v1.2.8` | Paid Report Run and Delivery Artifact Closure | Paid Report artifact / evidence / decision / delivery |
...
### v1.2.8 - Paid Report Run and Delivery Artifact Closure

`v1.2.8` 继续 `v1.2.7`，把 generic Paid Report Flow 从 admitted handoff / run contract 推进到可审计的 report delivery artifact closure：

- release certification 绑定实际 `v1.2.8` tag / release facts，并记录 tag policy；
- Paid Report product instance id 必须包含 project/workspace identity；
- input snapshot 和 order intent 必须在 run execution 前就绪；
- run execution receipt 记录 started / completed / blocked 事实；
- report artifact schema 只定义 generic report artifact，不引入具体行业 SKU；
- evidence pack 绑定 input snapshot、run receipt、report artifact 和 generation receipt；
- decision gate 控制 accepted / needs-fix / rejected / deferred / blocked；
- delivery package projection 只读，不写 authority；
- feedback / repair request 只能创建受控 follow-up route，不修改已交付 artifact。

本版仍不做 payment provider、checkout、账号、真实 report generation、具体 report SKU 或公开商业上线。
