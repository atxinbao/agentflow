...
```text
crates/**     = Core OS Runtime，只放通用内核能力
products/**   = Industry Product Surface / Reference App source
apps/**       = 用户界面，只消费 Projection / Product Surface / Runtime API
docs/**       = 人类可读项目文档、架构合同、需求和交付记录
.agentflow/** = Runtime 事实源，不是源码产品定义
```
...
- `apps/**` 展示和触发规则；
- `docs/**` 解释规则；
- `.agentflow/**` 记录规则执行后的事实。

Software Dev 可以作为 first-party built-in Reference App 随 AgentFlow 一起发布，但源码边界上必须属于 `products/software-dev/**`，不能成为 Core OS Runtime 的一部分。

详细合同见 [../architecture/086-industry-product-source-boundary-v1.md](../architecture/086-industry-product-source-boundary-v1.md)。
...
当前 `crates/pack/fixtures/packs/software-dev/**` 只能作为测试夹具或迁移输入，不应继续作为正式行业壳源码位置。
