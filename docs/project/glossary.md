# Glossary

更新日期：2026-06-26
执行者：Codex

| 术语 | 定义 |
| --- | --- |
| AgentFlow | Spec-Driven Software Dev Workflow 产品 |
| AI OS Project | Core Runtime 加行业 AgentFlow Product 的项目操作系统形态 |
| Core Runtime | Spec、Ontology、Runtime、Evidence、Decision、Projection 六个 Kernel 的底层能力 |
| Industry AgentFlow Product | 行业壳，由 Domain Pack、Surface Pack、Connector Pack 和行业工作流组成 |
| Software Dev Product | 当前唯一商业产品主线 |
| Spec Bundle | 经确认的需求、计划、任务、证据、判定和交付合同 |
| Domain Pack | 行业对象、动作、关系和状态定义 |
| Surface Pack | 行业页面、读模型、命令入口和工作台定义 |
| Connector Pack | 外部工具、provider、capability 和执行器连接定义 |
| Evidence | 证明发生过什么的材料，例如 diff、test log、PR、release、截图 |
| Decision | Done / rejected / deferred / needs-fix 等完成判定 |
| Delivery | PR、release、handoff、decision record 或交付包 |
| Audit Sidecar | 独立审计流程，不属于默认业务主链 |
| Runtime Authority | `.agentflow/**` 中由 AgentFlow Runtime 维护的执行事实源 |
| Projection | 从 authority 派生给 UI 和第三方读取的只读视图 |
