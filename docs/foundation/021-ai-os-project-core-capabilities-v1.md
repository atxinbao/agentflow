...
```text
AgentFlow Core = Spec-driven Project Runtime + Ontology + Event/Evidence + Acceptance + Projection
```
...
`.agentflow/` 面向 Agent、Runtime、Projection、Acceptance Gate 和 Audit Agent。
...
| Evidence Kernel | 记录命令、输出、artifact、diff、日志、截图、证明和 provenance | 不替代 Acceptance 决策 |
| Acceptance Kernel | 根据 Spec + Evidence 判断 accepted / rejected / deferred / blocked | 不执行实现任务 |
...
```text
Requirement / Goal / PRD / Spec / Architecture Plan / Issue / Run / Evidence / Acceptance / PR / Release / Feedback
```
...
只有 Acceptance Gate 通过，才允许 Completion Commit。
...
| --- | --- |
| Spec Bundle | PRD / technical plan / issues / acceptance / delivery |
...
| Acceptance Gate | tests pass + scope respected + evidence complete + delivery ready |
...
026-acceptance-kernel-contract-v1.md
